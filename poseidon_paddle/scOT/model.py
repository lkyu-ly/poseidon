...
class ScOTLayer(paddle.nn.Layer):
    ...
    def get_attn_mask(self, height, width, dtype):
        ...
        # to_static(PIR) 下 attn_mask 是 pir.Value，跨 train/eval Program 缓存会致 clone_program
        # IRMapping 丢键（根因：cache_key 不含 Program 身份）。仅动态图读写缓存，静态 trace
        # 各 Program 自算 mask，避免跨 Program Value 污染。
