...
# CINN 单一开关：必须在 import paddle 之前设置 FLAGS（踩坑手册坑四）。
# 开(=1)：三 FLAGS 全 true，train.py 后续对 model 做 paddle.jit.to_static，CINN 实质生效。
# 关/未设(=0)：三 FLAGS 全 false，不做 to_static，回到原纯动态图（minimal_train.sh 行为）。
