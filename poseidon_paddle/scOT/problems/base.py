...
def get_dataset(dataset, **kwargs):
    ...
    **kwargs overwrite the default settings.
    ...
    if isinstance(dataset, list):
        return paddle.io.ConcatDataset([get_dataset(d, **kwargs) for d in dataset])
...
class BaseDataset(paddle.io.Dataset, ABC):
    ...
    @property
    def reader(self):
        """Open HDF5 files lazily per process to avoid worker crashes."""
        if self._reader_path is None:
            return self._reader

        current_pid = os.getpid()
        if self._reader is None or self._reader_pid != current_pid:
            if self._reader is not None:
                self._reader.close()
            self._reader = h5py.File(self._reader_path, "r")
            self._reader_pid = current_pid
        return self._reader

    @reader.setter
    def reader(self, value):
        if self._reader is not None and self._reader is not value:
            self._reader.close()
        if isinstance(value, str):
            self._reader = None
            self._reader_path = value
            self._reader_pid = None
        else:
            self._reader = value
            self._reader_path = getattr(value, "filename", None)
            self._reader_pid = os.getpid() if value is not None else None
    ...
    def _move_to_local_scratch(self, file_path):
        if self.move_to_local_scratch is not None:
            ...
            if not os.path.exists(dest_dir) and rank == 0:
    ...
    def get_channel_lists(label_description):
        matches = re.findall("\\[([^\\[\\]]+)\\]", label_description)
